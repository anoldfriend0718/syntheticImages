...
int main(int argc, char *argv[])
{
    ...
    int nx_vox = atoi(argv[2]), ny_vox = atoi(argv[3]);
    printf("dimensions: nx: %d  ny: %d \n",nx_vox,ny_vox);
    
    int     num_gc = 3;
    
    /* n[3] refers to dimensions of fill box */
    n[0] = nx_vox; n[1] = ny_vox; 
    nx = n[0]; nxy = nx*n[1];
    ...
    data = (unsigned char *)malloc((nx_vox*ny_vox)*sizeof(unsigned char));
    fread(data, sizeof(unsigned char), (nx_vox*ny_vox), fp); 
    fclose(fp);
    
    Grid *g;
    /* x_lo refers to fill box. For ghostbox, it is x_lo_ghostbox */
    x_lo[0] = 0 + num_gc*options->dx; x_hi[0] = x_lo[0] + nx_vox*options->dx;
    x_lo[1] = 0 + num_gc*options->dx; x_hi[1] = x_lo[1] + ny_vox*options->dx;
    ...
    for( j = g->jlo_fb; j <= g->jhi_fb; j++)
    {
        ...
        {
            idx = (i - num_gc) + (j - num_gc)*nx;
            idx_gb = i + j*nx_gb;
            if( data[idx] == 255)  p->mask[idx_gb] =  options->dx;  //grain space, mask is positive
            else                   p->mask[idx_gb] = -options->dx; //pore space, mask is negatr
            
        }
    } 

    printf("final idx: %d\n", idx);
    printf("final idx_gb: %d\n", idx_gb);
    
    qss_reinitialize_mask2d(p, g, options);

    sprintf(fname,"mask");
    writeDataArrayQSS(p->mask,g,fname,GZIP);
    printf("mask is outputted \n");
